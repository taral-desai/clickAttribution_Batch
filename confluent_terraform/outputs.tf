output "resource-ids" {
  ...
  Kafka topic name1: ${confluent_kafka_topic.clicks.topic_name}
  Kafka topic name2: ${confluent_kafka_topic.checkouts.topic_name}
  ...
  In order to use the Confluent CLI v2 to produce and consume messages from topic '${confluent_kafka_topic.clicks.topic_name}' using Kafka API Keys
  ...
  # 2. Produce key-value records to topic '${confluent_kafka_topic.clicks.topic_name}' by using ${confluent_service_account.app-producer.display_name}'s Kafka API Key
  $ confluent kafka topic produce ${confluent_kafka_topic.clicks.topic_name} --environment ${confluent_environment.staging.id} --cluster ${confluent_kafka_cluster.basic.id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}"
  ...
  # 3. Consume records from topic '${confluent_kafka_topic.clicks.topic_name}' by using ${confluent_service_account.app-consumer.display_name}'s Kafka API Key
  $ confluent kafka topic consume ${confluent_kafka_topic.clicks.topic_name} --from-beginning --environment ${confluent_environment.staging.id} --cluster ${confluent_kafka_cluster.basic.id} --api-key "${confluent_api_key.app-consumer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-consumer-kafka-api-key.secret}"
